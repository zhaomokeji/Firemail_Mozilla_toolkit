...
; this installed.
!define FallbackKey \
  "SOFTWARE\Chunhuitech\MaintenanceService\3932ecacee736d366d6436db0f55bce4"
...
!define MaintFullName "Chunhuitech Maintenance Service"
!ifdef BrandFullName
!undef BrandFullName
!endif
...
InstallDirRegKey HKLM "Software\Chunhuitech\MaintenanceService" ""
...
Section "MaintenanceService"
  ...
  WriteRegStr HKLM "${MaintUninstallKey}" "Publisher" "Chunhuitech"
  ...
  ${OrIf} ${IsNativeARM64}
    SetRegView 64
  ${EndIf}
  WriteRegDWORD HKLM "Software\Chunhuitech\MaintenanceService" "Attempted" 1
  WriteRegDWORD HKLM "Software\Chunhuitech\MaintenanceService" "Installed" 1
  DeleteRegValue HKLM "Software\Chunhuitech\MaintenanceService" "FFPrefetchDisabled"

  ; Included here for debug purposes only.  
  ; These keys are used to bypass the installation dir is a valid installation
  ; check from the service so that tests can be run.
  WriteRegStr HKLM "${FallbackKey}\0" "name" "Chunhuitech Corporation"
  WriteRegStr HKLM "${FallbackKey}\0" "issuer" "DigiCert SHA2 Assured ID Code Signing CA"
  WriteRegStr HKLM "${FallbackKey}\1" "name" "Chunhuitech Fake SPC"
  WriteRegStr HKLM "${FallbackKey}\1" "issuer" "Chunhuitech Fake CA"
  ${If} ${RunningX64}
  ${OrIf} ${IsNativeARM64}
    SetRegView lastused
  ${EndIf}
...
Section "Uninstall"
  ...
  ${OrIf} ${IsNativeARM64}
    SetRegView 64
  ${EndIf}
  DeleteRegValue HKLM "Software\Chunhuitech\MaintenanceService" "Installed"
  DeleteRegValue HKLM "Software\Chunhuitech\MaintenanceService" "FFPrefetchDisabled"
